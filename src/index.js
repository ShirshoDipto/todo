...
function makeListActive(e) {
  ...
  if (e.target.classList.value === "delete") {
    ...
    changeTodoHeader(
      todoHeader,
      listNumber,
      this.firstElementChild.lastElementChild.textContent
    );
    const allTodos = Array.from(document.querySelectorAll(".a-todo"));
...
function showRelatedTodos(x, listNo) {
  if (lists.length === 0) {
    ...
    const activeList = document.querySelector(
      `.all-list div[list-number="${parseInt(listNo) - 1}"]`
    );
    activeList.setAttribute("id", "active-list");
...
function deleteElement(element) {
  if (element.classList.value === "a-list") {
    ...
    activeListNumber = document
      .querySelector("#active-list")
      .getAttribute("list-number");
  }
  saveData(lists, activeListNumber);
}
...
function updateDatas(theTodo, e, todoDOM) {
  ...
  theTodo.setPriority =
    e.target.firstElementChild.nextElementSibling.nextElementSibling.lastElementChild.value;
  theTodo.setDescription =
...
function loadPage(anArray) {
  ...
  const activeList = document.querySelector(
    `.all-list div[list-number="${activeListNumber}"]`
  );
  // removeExistingMarker(allList);
  if (activeList !== null) {
    activeList.setAttribute("id", "active-list");
  ...
  changeTodoHeader(
    todoHeader,
    activeListNumber,
    lists[parseInt(activeListNumber)].getTitle
  );
...
if (localStorage.getItem("activeListNumber")) {
  ...
  const activeList = document.querySelector(
    `.all-list div[list-number="${defaultList.getListNumber}"]`
  );
  activeList.setAttribute("id", "active-list");
  saveData(lists, activeListNumber);
}
